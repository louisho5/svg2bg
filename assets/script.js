...
function handleUpload(event) {
  ...
  reader.onload = (e) => {
    ...
    if (btnBgOnly.checked) {
      textarea.value =
        'background-image: url("data:image/svg+xml,' + temp + '");';
    }
...
function handleURL() {
  ...
  request.onload = function () {
    ...
    reader.onload = function (e) {
      var temp = e.target.result;
      temp_result = encodeXml(temp).replace(/  +/g, " ");
      var resultVal = "";
      resultVal += 'content: "";\n';
      ...
      resultVal += "background-position: center center;\n";
      resultVal +=
        'background-image: url("data:image/svg+xml,' + temp_result + '");';
      textarea.value = resultVal;
      if (btnBgOnly.checked) {
        textarea.value =
          'background-image: url("data:image/svg+xml,' + temp_result + '");';
...
function realTimeToggleBgOption() {
  if (textarea.value == "") {
    //nothing
  } else if (textarea.value.indexOf("center center;") < 0) {
    var resultVal = "";
    resultVal += 'content: "";\n';
    resultVal += "display: inline-block;\n";
    resultVal += "width: 100px;\n";
    ...
    resultVal += "background-repeat: no-repeat;\n";
    resultVal += "background-position: center center;\n";
    textarea.value = resultVal + textarea.value;
  } else {
    var resultVal = textarea.value.split(
      "background-position: center center;\n"
    );
    textarea.value = resultVal[1];
  }
}
